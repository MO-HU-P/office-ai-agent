...
- **Native charts, not screenshots (Excel & PowerPoint)** — column/bar/line/pie charts are inserted as real, editable chart objects bound to cell ranges, so edited numbers update the chart and the user can restyle it in Excel/PowerPoint. Error bars (e.g. mean ± SD) are supported via `error_bars_range`. matplotlib PNGs are reserved for Word figures and for chart types outside those four (box plots, histograms, …). Series colors are written into the file from a CVD-validated palette so the preview and the real app agree
- **Word** — documents with headings/lists/tables, batch edits, copying the look & feel of an existing document to another
- **Highlight keywords** — find every occurrence of given terms across a document and color/bold/underline just those characters (Word & PowerPoint apply it to the exact substring even when Word has split the text into fragmented runs; Excel applies it per matching cell)
- **PowerPoint** — multi-slide decks with shapes, images, tables, and charts. New decks are plain (16:9, white background) by default; when you ask for a designed look ("make it look nice", "use a template"), they are created from a bundled 16:9 design template instead (theme colors matching the app's Google-style palette, Japanese fonts, layout accents). A slide-design guide (`design_guide.md`: when to apply design, composition patterns, spacing rules, QA checklist) is auto-injected into the system prompt for PPTX requests only, and it also tells the agent never to restyle a deck you uploaded unless asked. Slide insertion adapts to third-party decks by picking a layout that actually has the needed placeholders (falling back to text boxes), and content-adding tools automatically push tables/charts/images/shapes below the slide title if the requested position would overlap it
- **Speaker notes** — read and write per-slide speaker notes (`ppt_read_notes` / `ppt_set_notes`): draft a talking-track from a slide's content, or turn a notes outline into slide text. With a vision model the agent looks at the rendered slide and writes notes to match what it sees. Notes appear in a collapsible panel below the slide in the preview
...
- **The previewed file is the default target** — messages that don't name a file operate on the file currently shown in the preview ("add one more slide" just works); the file name is attached to your message ("(表示中のファイル: …)"), and naming a different file in the message always takes precedence
...
- **Preview**: Excel via a custom grid, Word via docx-preview, PowerPoint via LibreOffice headless → PDF → PNG (per-slide speaker notes are extracted separately with python-pptx and shown in a collapsible panel below the slide, hidden when a slide has none). Embedded images and native charts are overlaid on the Excel grid at their anchor cell (the grid is extended to cover anchors placed outside the data range); charts are re-drawn as inline SVG from a spec extracted server-side (type, cell-resolved values, error bars, title, axes, size), which keeps cell selection working — unsupported chart types degrade to a labelled placeholder rather than vanishing
