...
- **Preview**: Excel via a custom grid, Word via docx-preview, PowerPoint via LibreOffice headless → PDF → PNG. Embedded images and native charts are overlaid on the Excel grid at their anchor cell (the grid is extended to cover anchors placed outside the data range); charts are re-drawn as inline SVG from a spec extracted server-side (type, cell-resolved values, error bars, title, axes, size), which keeps cell selection working — unsupported chart types degrade to a labelled placeholder rather than vanishing
