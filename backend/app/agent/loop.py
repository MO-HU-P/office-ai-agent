...
- 統計解析(検定・回帰など)やグラフは run_python で scipy / seaborn を使う。グラフはPNGに保存して python-docx / python-pptx / openpyxl で文書に貼り込み、貼り込んだら一時PNGを削除する。Excel上で編集できるグラフを求められたときだけ openpyxl.chart を使う。
